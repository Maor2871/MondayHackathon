    ...
    def update_boards_in_ws(self):
        ...
        for board in boards_names['boards']:
            ...
            boards_data = self.post_request(query='{ boards (ids:' + board[
                'id'] + ') {id name groups{id title} columns{id title type description} items{id name group{ id title } column_values{id text}} }}')
    ...
    def __init__(self, ws, name, exists=False, json_groups=None, json_columns=None, json_items=None, board_id=None):
        ...
        # Create the board on monday too.
            ...
            if not exists:
                ...
                # Create the query.
                query = 'mutation { create_board (board_name: "' + self.name + '", board_kind: private, workspace_id: ' + str(
                    self.work_space.work_space_id) + ') { id } }'
                ...
                for group in \
                        self.work_space.post_request(query='{ boards (ids: ' + self.board_id + ') {id groups{id title}} }')[
                            'boards'][0]['groups']:
    ...
    def manager(self):
        ...
        while True:
            ...
            items_json = self.work_space.post_request(
                query='{ boards (ids: ' + self.board_id + ') {id items{id name group {id title} column_values {title value}}} }')[
                'boards'][0]['items']
                ...
                if not current_item['column_values'][0]['value']:
                    ...
                    self.work_space.post_request(
                        query='mutation { change_column_value (board_id: ' + self.board_id + ', item_id: ' +
                              current_item[
                                  'id'] + ', column_id: "' + self.status_column_id + '", value: "{\\\"index\\\" : 0}") { id } }')

                    # Call the function that handles the item submission as a thread.
                    analyser = Analayser(input_board=self, item_id=current_item['id'],
...
class Analayser(MyThread):
    ...
    def manager(self):
        ...
        self.input_board.work_space.post_request(
            query='mutation { change_column_value (board_id: ' + self.input_board.board_id + ', item_id: ' + self.item_id + ', column_id: "' + self.input_board.status_column_id + '", value: "{\\\"index\\\" : 1}") { id } }')
    ...
    def __init__(self, board, title, group_id=None):
        ...
        # The group does not exist in monday.
            ...
            self.group_id = self.board.work_space.post_request(
                query='mutation { create_group (board_id: ' + self.board.board_id + ', group_name: "' + self.title + '") { id } }')[
                'create_group']['id']
    ...
    def __init__(self, board, title, description, column_type, column_id=None):
        ...
        # Create the column in monday.
            ...
            self.column_id = self.board.work_space.post_request(
                query='mutation{ create_column(board_id: ' + self.board.board_id + ', title:"' + self.title + '", description: "' + self.description + '", column_type:' + self.column_type + ') { id title description } }')[
                'create_column']['id']
...
class Item():
    ...
    def __init__(self, group, name, item_id=None, json_columns_values=None, columns_values=[]):
        ...
        # Update the item in monday.
            ...
            self.item_id = self.group.board.work_space.post_request(
                query='mutation {create_item (board_id: ' + self.group.board.board_id + ', group_id: "' + self.group.group_id + '", item_name: "' + self.name + '", column_values: "' + columns_values_json + '") { id } }')[
                'create_item']['id']
    ...
    def upload_file(self, column_title, file_path):
        ...
        query = 'mutation ($file: File!) { add_file_to_column (file: $file, item_id: ' + self.item_id + ', column_id: "' + \
                self.group.board.columns[column_title].column_id + '") {id }}'
    ...
    def add_link(self, column_title, link, description=''):
        ...
        if description:
            query = 'mutation { change_column_value (board_id: ' + self.group.board.board_id + ', item_id: ' + self.item_id + ', column_id: "' + \
                    self.group.board.columns[
                        column_title].column_id + '", value: "{\\\"url\\\":\\\"' + link + '\\\",\\\"text\\\":\\\"' + description + '\\\"}") { id } }'
        else:
            query = 'mutation { change_column_value (board_id: ' + self.group.board.board_id + ', item_id: ' + self.item_id + ', column_id: "' + \
                    self.group.board.columns[
                        column_title].column_id + '", value: "{\\\"url\\\":\\\"' + link + '\\\",\\\"text\\\":\\\"' + link + '\\\"}") { id } }'
